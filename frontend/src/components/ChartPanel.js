...
function toCandleData(c) {
    return { time: Math.floor(c.t / 1000), open: c.o, high: c.h, low: c.l, close: c.c };
}
function fillToMarker(ev) {
    if (ev.category !== "trade" || !ev.price)
        return null;
...
export default function ChartPanel() {
    ...
    const avgEntryLineRef = useRef(null);
    ...
    useEffect(() => {
        ...
        series.applyOptions({
            autoscaleInfoProvider: (original) => {
                ...
                const extras = [];
                if (pos.avg_entry > 0)
                    extras.push(pos.avg_entry);
                if (pos.mark > 0)
                    extras.push(pos.mark);
                if (extras.length === 0)
                    return base;
                return {
                    ...base,
    ...
    return (_jsxs("div", { className: "pane relative overflow-hidden h-full flex flex-col", children: [_jsxs("div", { className: "flex items-center justify-between px-4 py-2 border-b border-border/70 bg-panel/80 backdrop-blur flex-none", children: [_jsxs("div", { className: "flex items-center gap-3", children: [_jsx("span", { className: "pane-heading", children: "HYPE \u00B7 1m \u00B7 zoomed to trades" }), _jsxs("span", { className: "text-subtle text-xs font-mono", children: [candles.length, " candles \u00B7 ", markers.length, " markers"] })] }), _jsxs("div", { className: "flex items-center gap-3 text-xs font-mono", children: [_jsx(LegendDot, { color: UP, label: "entry" }), _jsx(LegendDot, { color: DOWN, label: "close" }), _jsx(LegendDot, { color: AVG_LINE, label: "avg entry", dashed: true }), _jsx(LegendDot, { color: MARK_LIN, label: "mark", dotted: true })] })] }), _jsx("div", { ref: containerRef, className: "flex-1 min-h-0" })] }));
}
function LegendDot({ color, label, dashed, dotted }) {
    return (_jsxs("span", { className: "flex items-center gap-1.5 text-subtle", children: [_jsx("span", { className: "inline-block w-4 h-[2px] rounded", style: {
